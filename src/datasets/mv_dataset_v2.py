...
class ShapeNet55V2(Dataset):
    def __init__(self, root, category, split, transform=None, tgt_transform=None,
        ...
        if isinstance(category, list):
            ...
            self.item_root = [os.path.join(root, eachCat) for eachCat in category]
            self.classes = sorted([synsetid_to_cate[eachCat] for eachCat in category])
        else:
            self.config = [os.path.join(root, f'{category}_{split}.txt')]
            self.item_root = [os.path.join(root, category)]
        ...
        for eachConf, eachRoot, eachClass in zip(self.config, self.item_root, self.classes):
            with open(eachConf, 'r') as f:
                ...
                for filename in lines:
                    ...
                    self.x.append(views)
                    self.z.append(self.class_to_idx[eachClass])
    ...
    def __getitem__(self, index):
        ...
        return views, point_set, self.z[index], self.z[index]
